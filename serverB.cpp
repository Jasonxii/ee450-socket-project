...
char id[SINGLESIZE];
char message_received[SINGLESIZE];
...
void read_file(){
	...
	FILE *fp;
	char id[MAXSIZE];
	fp = fopen("database_b.csv","r");
	message_send[0] = '\0';
	
	while(fgets(buffer, MAXSIZE, (FILE*) fp)){
		...
		if(compare_id(id,message_received) == true){
			...
			strncpy(message_send, buffer, MAXSIZE);
...
server_addr.sin_port = htons(SERVERB_PORT);
...
int n;
socklen_t len;
// receive the link info from the aws
while(true){
